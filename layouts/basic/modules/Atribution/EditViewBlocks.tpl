						...
						<div class="c-panel__body c-panel__body--edit blockContent js-block-content {if $IS_HIDDEN}d-none{/if}"
							...
							<div class="row">
                                ...
                                {foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
                                    ...
                                    {/if}
									<div class="{if $FIELD_MODEL->get('label') eq "FL_REAPEAT"} col-sm-4
								...
								{else} col-md-12 m-auto{/if} fieldRow row form-group align-items-center my-1">
										...
										<div class="{$WIDTHTYPE} w-100 {if $FIELD_MODEL->getUIType() neq "300"} col-lg-12 col-xl-9 {/if} fieldValue" {if $FIELD_MODEL->getUIType() eq '19' or $FIELD_MODEL->getUIType() eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1}{elseif $FIELD_MODEL->getUIType() eq '300'} colspan="4" {assign var=COUNTER value=$COUNTER+1} {/if}>
                                            ...
                                            {foreach item=DETAIL from=$DETAILS name=listview}
                                                {if $FIELD_NAME == "rubriques" }
                                                    {*<input class="col-xl-12 form-control"   value=" {$DETAIL["rubrique"]}" disabled=""><br>*}
													<textarea name="rubriques" id="Atribution_editView_fieldName_rubriques" class="form-control " title="Rubriques "
													data-validation-engine="validate[funcCall[Vtiger_Base_Validator_Js.invokeValidation]]"  disabled>{$DETAIL["rubrique"]}</textarea>
                                                {/if}
                                                {if $FIELD_NAME == "prestations" }
													<textarea name="prestations" id="Atribution_editView_fieldName_prestations" class="form-control " title="Prestations "
													data-validation-engine="validate[funcCall[Vtiger_Base_Validator_Js.invokeValidation]]"></textarea>
                                                {/if}

                                                {if $FIELD_NAME == "villes" }
													<input class="col-xl-12 form-control" value=" {$DETAIL["ville"]}" disabled=""/><br>
                                                {/if}
                                                {if $FIELD_NAME == "regions" }
													<input class="col-xl-12 form-control" value=" {$DETAIL["region"]}" disabled=""/><br>
                                                {/if}

                                                {if $FIELD_NAME == "prestation_supp" }
													<select class="col-xl-12 form-control" id="prestation_suppSelect2" multiple="multiple"></select>
                                                {/if}
                                            {/foreach}
