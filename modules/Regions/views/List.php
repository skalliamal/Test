...
class Regions_List_View extends Vtiger_List_View
{
    ...
    public function syncroniseAccountsAdded()
    {
        ...
        $regions=$mysqli->query("SELECT * from region ");

        $tableName="vtiger_crmentity";
        $rows = $regions->fetch_all();
       /* var_dump($rows);
        die('stopped here');*/
        ...
        foreach ($rows as $row) {
           ...
           /* var_dump($tableData);
            ...
            $db->createCommand()->insert("u_yf_regions", ["regionsid" => $LastInsertID] + $tableData)->execute();

        }
        $mysqli->close();
        header('Location: index.php?module=Regions&view=List&mid=197&parent=0');
    }
