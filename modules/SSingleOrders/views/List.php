...
class SSingleOrders_List_View extends Vtiger_List_View
{
    ...
    public function syncroniseAccountsAdded()
    {
        ...
        $orders=$mysqli->query("SELECT * FROM FIC_ORDRE WHERE `DORD` BETWEEN '20100101' AND '20190228' ORDER BY `FIC_ORDRE`.`DORD` DESC");
       ...
       /*  var_dump($rows);
        ...
        foreach ($rows as $row) {
            ...
            $tableData=array(
                ...
                "firme"=> $row[2],
                "signataire"=> $row[11],
                "nom_signataire"=> $row[12],
                "sum_gross"=> $row[24],
                "docs_pub_recus"=> $row[49],
                "validation_bc"=> $row[55],
                "nouveau"=> $row[47],
                "ordre_facture"=> $row[50],
                "date_validation"=> $row[56],
            );
           /* var_dump($tableData);
            die('stopped here');*/
            ...
            $db->createCommand()->insert("u_yf_ssingleorders", ["ssingleordersid" => $LastInsertID] + $tableData)->execute();

        }
        $mysqli->close();
        header('Location: index.php?module=SSingleOrders&view=List&mid=124&parent=163');
    }
    public function getEntityDataForSave()
    {
        $row = [];
        $time = date('Y-m-d H:i:s');
        $row['setype'] = "SSingleOrders";
