	...
	public function initializeListViewContents(\App\Request $request, Vtiger_Viewer $viewer)
	{
		...
		if (!$this->listViewEntries) {
            $this->listViewEntries = $this->listViewModel->getListViewEntries($pagingModel);
			    ...
			    $this->listViewEntries=$mysqli->query("SELECT cmd.ssingleordersid,cmd.num_ordre,cmd.num_bc,cmd.support,cmd.type_ordre,cmd.date_ordre,cmd.edition,cmd.raison_sociale,emp.name,cmd.nom_signataire
			                FROM u_yf_ssingleorders cmd
			                INNER JOIN vtiger_crmentity c on cmd.ssingleordersid = c.crmid 
                             INNER JOIN vtiger_ossemployees emp on cmd.commercial = emp.responsable 
			                where (select count(*) from u_yf_ssingleorders_inventory inv where inv.id=cmd.ssingleordersid)> (select count(*) from u_yf_atribution attr where attr.ordre=cmd.ssingleordersid) ORDER BY cmd.ssingleordersid")->fetch_all();
