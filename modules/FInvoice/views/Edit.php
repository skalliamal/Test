    ...
    public function process(\App\Request $request)
    {
        ...
        if (!empty($record) && $request->getBoolean('isDuplicate') === true) {
            ...
            if ($referenceId) {
                ...
                $finvoiceid=(new App\Db\Query())->select(['finvoiceid'])->from('u_yf_finvoice')->where(['ssingleordersid' => $referenceId])->one()['finvoiceid'];
                if($finvoiceid) {
                    if ($request->getInteger('generateReglement')) {
                        $loadUrl = "index.php?module=Reglements&view=Edit&reference_id=" . $finvoiceid;
                        header("Location: $loadUrl");
                    } else {
                        $loadUrl = "index.php?module=FInvoice&view=Detail&record=" . $finvoiceid;
                        header("Location: $loadUrl");
                    }
                }
