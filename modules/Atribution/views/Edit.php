...
class Atribution_Edit_View extends Vtiger_Index_View
{
    /**
    ...
    public function process(\App\Request $request)
    {
        ...
        $get_code_produit=(new \App\Db\Query())->select("serviceid")
            ->where(["code_produit"=>$code_produit])
            ->from("vtiger_service ")->all();
        var_dump($get_code_produit);

        foreach ($get_code_produit as &$code_produits) {

        }

        /* $ML=(new \App\Db\Query())->select("*")
             ->where(["name"=>$code1])
             ->from("u_yf_ssingleorders_inventory ")->distinct()->all();
         var_dump($ML);*/
        $record=$request->get('record');
        // var_dump($num_ord);

        $ordre=(new \App\Db\Query())->select("ordre")
            ->where(["atributionid"=>$record])
            ->from("u_yf_atribution ")->distinct()->all();
        //var_dump($ordre);
        foreach ($ordre as &$ordr) {
        }
        //var_dump($ordr);
        $ML=(new \App\Db\Query())->select("r.rubrique,v.ville,re.region")
            ->leftJoin('vtiger_service S','S.serviceid=i.name')
            //->leftJoin('u_yf_atribution a','a.ordre=i.id')
            ->leftJoin('u_yf_rubriques r','r.rubriquesid=i.ref')
            ->leftJoin('u_yf_villes v','v.villesid=i.value28')
            ->leftJoin('u_yf_regions re','re.regionsid=i.value26')
            ->where(["S.code_produit"=>$code_produit])
            ->andWhere(["i.id"=>$ordr])
            //->andWhere(["i.name"=>$ordre])
            //->andwhere(["in","i.name",$ordre])
            ->from("u_yf_ssingleorders_inventory i")->all();
        var_dump($ML);

        //SELECT ref FROM u_yf_ssingleorders_inventory,vtiger_service WHERE name=vtiger_service.serviceid AND vtiger_service.code_produit='ML'
